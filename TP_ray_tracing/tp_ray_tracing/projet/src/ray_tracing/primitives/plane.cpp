...
bool plane::intersect(ray const& ray_param,intersection_data& intersection) const
{
    ...
    //vec3 const& ray_param.p0() + relative*ray_param.u;

    //dot(u,vec3(-2.0f*n.y()+n.x(),n.y(),n.z()));
    // float t_inter = ray_param.p0() + position_data*ray_param.u();
    vec3 const& n_inter = n;
    float t_inter = dot((position_data - ray_param.p0()),n_inter) / dot(u,n_inter); //relative param
    vec3 const& p_inter = ray_param.p0()+t_inter*ray_param.u(); // position de l'intersection rayon-plan


    // float const p=dot(u,vec3(-2.0f*n.y()+n.x(),n.y(),n.z()));
    // float const t=2.0f*p*dot(normalized(position_data)-n,n);
    //2.0f*p*dot(normalized(position_data)-n,n);
